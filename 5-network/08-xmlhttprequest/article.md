...
요즘은 `fetch`라는 더 현대적인 방식이 있어 `XMLHttpRequest`를 어느 정도 대체합니다.
...
2. 일반적으로 `new XMLHttpRequest`로 객체를 생성한 직후 초기화합니다.
    ```js
...
[명세서](https://xhr.spec.whatwg.org/#states)에 따른 모든 상태는 다음과 같습니다.
...
: 주어진 `name`과 `value`로 요청 헤더를 설정합니다.
    ...
    전체 목록은 [명세서](https://xhr.spec.whatwg.org/#the-setrequestheader()-method)에서 확인할 수 있습니다.
...
: `Set-Cookie`와 `Set-Cookie2`를 제외한 모든 응답 헤더를 반환합니다.
    ...
    헤더 사이의 줄 바꿈은 항상 `"\r\n"`입니다(OS와 무관). 그래서 개별 헤더로 쉽게 나눌 수 있습니다. 이름과 값 사이의 구분자는 항상 콜론 뒤에 공백이 붙은 `": "`입니다. 이는 명세서에 고정되어 있습니다.
...
실제로는 이벤트가 더 많습니다. [최신 명세서](https://xhr.spec.whatwg.org/#events)에는 수명 주기 순서대로 다음 이벤트가 나열되어 있습니다.
...
`readystatechange`라는 이벤트도 이미 살펴봤습니다. 역사적으로 이 이벤트는 명세서가 자리 잡기 전 오래전에 등장했습니다. 요즘은 사용할 필요가 없고, 더 새로운 이벤트로 대체할 수 있지만 오래된 스크립트에서는 자주 볼 수 있습니다.
