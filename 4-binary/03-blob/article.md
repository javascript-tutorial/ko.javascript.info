...
다음과 같이 `Blob` 객체에 slice 메서드를 적용해 일부를 추출할 수 있습니다.
...
`URL.createObjectURL`을 통해 생성된 URL은 브라우저가 내부적으로 URL을 `Blob`으로 매핑(mapping)하는 과정을 거쳐 저장합니다. 그래서 만들어진 URL이 짧음에도 불구하고 `Blob`에 접근이 가능합니다.

생성된 URL(그리고 생성된 링크를 포함해서)은 현재 열린 HTML document에서만 유효합니다. 그리고 URL을 필요로하는 `<img>`, `<a>`, 등 다른 태그에서도 `Blob`을 참조하는 것이 가능합니다.
...
`Blob`을 이용해서 이미지 일부를 저장하거나, 페이지 스크린 샷을 만들 수도 있습니다.

이미지 연산은 HTML의 `<canvas>` 요소를 통해 완성됩니다:
...
다음 예시는 Blob을 만들기 전에 단순히 이미지를 복사한 것입니다. Blob이 아니어도 canvas에서 이미지 수정·변환이 가능합니다.
...
`async/await` 방식을 사용하는 경우:
```js
let blob = await new Promise(resolve => canvasElem.toBlob(resolve, 'image/png'));
```

페이지의 스크린샷을 찍고 싶다면, 이 라이브러리(<https://github.com/niklasvh/html2canvas>)를 사용하면 됩니다. 이 기능은 단순히 페이지를 쭉 훑어가면서 `<canvas>`에 그리는 것입니다. 그러면 우리도 위 예시와 같은 방법으로 페이지의 `Blob`을 만들 수 있습니다.
