...
* 공백(스페이스), 큰따옴표("), 작은따옴표('), 대시(-), 백틱(\`)을 비롯한 모든 특수문자는 수정하시면 안 됩니다. **자연어만 수정해주세요!** 많은 분들이 렌더링 결과를 확인하지 않고 자의대로 특수문자를 지우거나 수정하셔서 이 부분을 리뷰하는 데 상당한 시간이 걸리고 있습니다. **특수문자 수정 등으로 인하여 렌더링시 깨지는 경우 PR 받지 않겠습니다.**
