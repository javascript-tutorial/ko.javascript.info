...
조절점의 개수는 2개나 3개, 4개가 될 수 있고 이보다도 더 많을 수 있습니다.
...
2. 조절점 1과2, 2와 3을 잇는 선분(<span style="color:#825E28">갈색</span>선)을 만듭니다.
...
4. 이제 <span style="color:#167490">파란색</span> 선분에서 `t` 값과 비례하는 위치에 점 하나를 찍습니다. `t`가 `0.25`일 때(왼쪽 그림)는 파란 선분의 4분의 1, `t`가 `0.5`일 때(오른쪽 그림)는 선분의 가운데에 점(<span style="color:red">빨간점</span>)이 생깁니다.
