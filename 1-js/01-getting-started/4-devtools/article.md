...
대부분의 개발자는 Chrome이나 Firefox를 이용해 개발하는 걸 선호합니다. 두 브라우저에서 제공하는 개발자 도구가 굉장히 훌륭하기 때문이죠. 기타 브라우저들도 개발자 도구를 제공하고 독특한 기능 있지만, 거의 Chrome이나 Firefox 기능을 "따라가는" 수준입니다. 그래서 개발자들은 크롬이나 파이어폭스 중 "선호하는" 브라우저를 하나 택해 개발하다가 사용하고 있는 브라우저에 종속된 문제가 발생하면 다른 브라우저로 전환해 개발을 이어나가곤 합니다.  
...
`key:F12`를 눌러봅시다. Mac 사용자라면 `key:Cmd+Opt+J`를 누르시면 됩니다.
...
우리는 대부분 한줄짜리 명령어를 입력하고 `key:Enter`를 눌러 해당 명령어를 실행하는 작업을 많이 합니다.
...
- Windows 사용자는 `key:F12`를 눌러 개발자 도구를 열 수 있습니다. Mac 사용자는 Chrome에선 `key:Cmd+Opt+J`, Safari에선 `key:Cmd+Opt+C`를 누르면 됩니다. 사파리는 개발자 메뉴를 활성화 해 줘야 개발자 도구를 사용할 수 있습니다.
