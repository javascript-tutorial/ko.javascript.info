...
왜냐하면 함수는 엔진이 `try..catch` 구문을 떠난 후에야 실행되기 때문입니다.
...
지금까지 불완전한 데이터를 `try..catch`를 사용해 처리해 보았습니다. 그런데 *또 다른 예기치 않은 에러*가 `try {...}` 블록 안에서 발생하는 것이 가능할까요? 정의하지 않은 변수를 사용하려는 경우와 같이, 프로그래밍 에러가 발생할 가능성도 충분히 있을 수 있죠.
...
`try..catch`로 런타임 에러를 처리할 수 있습니다. "try"에서 코드를 실행하고, 에러가 발생하면 "catch"에서 잡아냅니다.
