...
여러 개의 조건을 처리해야 할 때도 있습니다. 이럴 때 `else if`를 사용할 수 있습니다. 
...
물음표 연산자를 이런 방식으로 쓰는 걸 처음 본 분이라면 이 코드가 어떻게 동작하는지 파악하기 힘들 수 있습니다. 그러나 주의를 집중하고 본다면, 단순한 조건의 나열임을 파악할 수 있습니다.
...
조건 `company == 'Netscape'`의 검사 결과에 따라 `?` 뒤에 이어지는 첫 번째 혹은 두 번째 표현식이 실행되고 출력을 보여줍니다.
