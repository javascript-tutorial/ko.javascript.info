...
이와 같이 데이터 타입(data type, 자료형)은 있지만, 변수가 어느 타입에도 묶여있지 않은 언어를 "동적 타입(dynamically typed)" 언어라 부릅니다.

자바스크립트에는 일곱 가지 기본 자료형이 있습니다. 이번 챕터에선 자료형에 대해 전반적으로 다루고, 이후 챕터에서 각 자료형을 자세히 다루도록 하겠습니다.
...
*숫자형(Number)* 은 정수 및 부동소수점 숫자를 나타냅니다.

숫자형과 관련된 연산엔 곱셈 `*`, 나눗셈 `/`, 덧셈 `+`, 뺄셈 `-` 등이 있습니다.

숫자형엔 일반 숫자 외에도 `Infinity`, `-Infinity`, `NaN`같은 "특수 숫자 값(special numeric values)"이 포함됩니다.
...
자바스크립트를 이용해 수행하는 수학 연산은 "안전"하다고 볼 수 있습니다. 0으로 나누거나 숫자가 아닌 문자열을 숫자로 취급하는 등의 모든 연산이 가능합니다.

말이 안되는 수학 연산을 하더라도 스크립트는 치명적인 에러를 내뿜으며 죽지 않습니다. 최악의 경우엔 `NaN`을 결과로 얻게 됩니다.
```

특수 숫자 값은 현실 세계에선 숫자가 아니지만, 공식적으로 "숫자"형에 속합니다.
...
`${…}` 내부의 표현식은 평가 후 문자열의 일부가 됩니다. `${…}` 안에는 무엇이든 들어갈 수 있습니다. `name` 같은 변수나 `1 + 2` 같은 수학관련 표현식 이외에도 복잡한 표현식을 넣을 수 있습니다. 

이런 방법은 역 따옴표를 써야만 유효합니다. 기본 따옴표에는 이러한 기능을 지원하지 않으므로 유의하시기 바랍니다.
```js run
...
```smart header="*글자*형은 없습니다."
일부 언어에는 글자 하나를 위한 "글자(character)"형이 있습니다. C 언어와 Java에서의 `char`가 그 대표적인 예입니다.

자바스크립트에는 글자형이 없습니다. `문자열`을 위한 자료형인 문자형만 있습니다. 문자열은 글자 하나 또는 많은 글자로 이루어져 있습니다.
```

## 불린형(논리 타입)

불린형은 `true`와 `false` 두 가지 값만 가집니다.

이 자료형은 긍정(yes)이나 부정(no)에 해당하는 값을 저장할 때 사용합니다. `true`는 "네. 맞습니다"를 의미하고 `false`는 "아니요, 틀립니다"를 의미하죠.
...
`null`은 단지 "존재하지 않는(nothing)", "비어 있는(empty)" 또는 "알 수 없는(unknown)" 값을 나타내는 특별한 값입니다.

위 코드는 어떤 이유때문에 `나이(age)`를 알 수 없거나 그 값이 비어있는 경우를 나타냅니다.

## "undefined" 값

`undefined` 값도 별도의 자료형을 만듭니다. `null`처럼 자신만의 고유한 자료형인 `undefined`형에 속하게 됩니다.

`undefined`는 "값이 할당되지 않은 상태"를 의미합니다.

변수는 선언되었지만, 값을 할당하지 않았다면 해당 변수는 `undefined`를 반환합니다.
...
모든 변수에 `undefined`를 할당하는 것이 가능합니다.
...
하지만 이렇게 `undefined`를 할당하는 걸 권장하지 않습니다. 변수가 "비어있거나" "알 수 없는" 상태라는 걸 나타내는 데는 `null`이 사용됩니다. `undefined`는 변수에 값이 할당되었는지 여부를 확인할 때 사용하죠.

## 객체와 심볼

`객체(object)`는 특별한 자료형을 만듭니다.

객체형을 제외한 다른 자료형은 문자열이든 숫자든 한 가지만 표현할 수 있기 때문에 원시(primitive) 자료형이라 부릅니다. 반면, 객체는 데이터 컬렉션과 복잡한 개체(entity)를 저장하도록 해줍니다. 객체에 대한 자세한 내용은 원시 자료형을 배우고 난 후 <info:object>에서 다루도록 하겠습니다.

`심볼(symbol)`형은 객체의 고유한 식별자(unique identifiers)를 만들 때 사용됩니다. 심볼형은 객체를 이해하고 난 이후에 학습하는 것이 낫기 때문에 넘어가도록 하겠습니다. 
...
- 정수 또는 부동 소수점 등의 모든 종류의 숫자를 나타내는 `숫자`형.
- 문자열을 위한 `문자`형. 문자열은 하나 혹은 그 이상의 문자로 구성되며, 단일 문자를 나타내는 별도의 자료형은 없음.
- `true`/`false`(참/거짓)를 나타내는 `불린`형.
...
- `"string"`과 같이 자료형을 문자열 형태로 반환합니다.
- `null`의 typeof 연산은 `"object"`인데, 이는 언어상의 오류입니다. null은 객체가 아닙니다. 

이어지는 챕터에서는 원시 자료형에 대해 먼저 살펴보고, 이 자료형이 익숙해지면 객체를 학습하도록 하겠습니다.
