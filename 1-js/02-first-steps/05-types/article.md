...
- `Infinity`는 어떠한 숫자보다 큰 특별한 값인 [무한대 ∞](https://en.wikipedia.org/wiki/Infinity)를 나타냅니다.
...
- `NaN`은 계산 중에 에러가 발생했다는 것을 나타내주는 값입니다. 부정확하거나 정의되지 않은 수학 연산을 사용하면 계산 중에 에러가 발생하는 데, 이때 `NaN`이 반환됩니다.
    ...
    ```js run
    alert( "not a number" / 2 ); // NaN, 문자열을 숫자로 나누면 오류가 발생합니다.
    ```
...
`null`과 비슷하게, `undefined` 값도 자신만의 자료형을 형성합니다.
