...
예를 들어, 안전한 정수의 범위를 살짝 벗어나는 다음 두 숫자는 같은 값으로 취급합니다.
...
보통 이런 괄호 안에는 `(2 + 2)`처럼 수식이 들어가지만, 여기서는 `(x)`처럼 인수 하나만 들어갑니다. 문법상 `typeof x` 대신 `typeof(x)`처럼 `typeof` 연산자 바로 뒤에 괄호를 붙여 쓸 수 있으며, 이를 선호하는 사람들도 있습니다.
