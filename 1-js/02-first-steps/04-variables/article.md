...
자바스크립트 애플리케이션은 작업에 필요한 정보와 함께 동작합니다. 아래와 같이 말이죠.
...
[변수](https://en.wikipedia.org/wiki/Variable_(computer_science))는 데이터를 저장할 때 쓰이는 "이름을 붙인 저장소" 입니다. 상품, 방문객 등의 데이터를 저장하기 위해 변수를 사용할 수 있습니다.
...
`var` 키워드는 `let`과 *거의* 같습니다. 변수를 선언하는 "오래된" 방식이죠.

`let`과 `var` 사이에는 미묘한 차이점이 있습니다. 둘 사이의 차이점은 지금 시점에선 중요하지 않습니다. 이에 대해서는 추후 <info:var> 에서 자세히 다루겠습니다.
````
