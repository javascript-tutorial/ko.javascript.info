...
- 연산자가 하나의 피연산자를 가진 경우 이것은 *unary*입니다. 예를 들어, unary negation(단항 부정 연산자) `-`는 숫자의 부호를 뒤집습니다:
    ...
    */!*
    ...
    ```
- 연산자가 2개의 피연산자를 가진 경우 *binary*입니다. 같은 뺄셈이지만 binary form(이항 형식)으로도 존재합니다:
...
그러나 binary `+`가 strings(문자열)에 적용되면, 그 문자열들을 병합(연결)합니다 :
...
피연산자 중 하나가 string이면 다른 하나도 string으로 변환됩니다.
...
첫 번째 피연산자가 string인지 또는 두 번째 피연산자가 string인지는 중요하지 않습니다. 규칙은 간단합니다. 피연사자중 어느 하나가 string이면 다른 하나도 string으로 변환됩니다.

그러나 연산은 왼쪽에서 오른쪽으로 실행됩니다. 두 개의 숫자 뒤에 string이 오면, 숫자들은 string로 변환되기 전에 덧셈 연산을 합니다.

```js run
alert(2 + 2 + '1' ); // "41" and not "221"
```

문자열 연결 및 변환은 binary 덧셈`+`의 특별한 기능입니다. 다른 산술 연산자는 숫자로만 계산하며 항상 피연산자를 숫자로 변환합니다.
...
수학자의 관점에서, 다수의 pluses(덧셈 기호)가 이상하게 보일지도 모릅니다. 그러나 프로그래머의 관점에서 보면 특별한 것은 없습니다. 단항연산자 pluses가 먼저 적용되면서 strings을 숫자로 변환 합니다. 그러고 나서 이항연산자 plus가 그것들을 합산합니다. 
...
- **Increment**(증가 연산자) `++`는 변수를 1씩 증가시킵니다.:
    ...
    alert( counter ); // 3
    ```
- **Decrement**(감소 연산자) `--`1씩 변수를 감소시킵니다. :
