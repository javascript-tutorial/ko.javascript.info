...
두 번째 import에서는 아무것도 출력되지 않습니다. 모듈 코드가 이미 한 번 실행되었기 때문입니다.

이때 기억해야 할 원칙이 있습니다. 최상위 레벨 모듈 코드는 초기화나 모듈 내부에서 사용하는 데이터 구조를 생성하는 용도로 사용해야 합니다. 여러 번 호출해야 하는 기능이 필요하다면 앞서 sayHi에서 살펴본 것처럼 함수로 내보내야 합니다.
...
*/!*
```
1.js에서 import한 admin 객체의 name 프로퍼티를 변경하면, 2.js에서도 변경된 admin.name 값을 확인할 수 있습니다.
...
보통은 아래와 같은 방식으로 모듈을 설정합니다.

1. 모듈이 설정 수단을 export합니다. 예를 들어 설정 객체를 export할 수 있습니다.
2. 최초 import 시 설정을 초기화하고 객체의 프로퍼티에 값을 할당합니다. 일반적으로 최상위 애플리케이션 스크립트에서 이를 수행합니다.
3. 이후의 import에서는 설정이 완료된 모듈을 그대로 사용합니다.
