...
상당수는 자바스크립트 내장 함수는 임의의 수의 인수를 허용합니다.
...
이번 챕터에서는 이렇게 임의의 수의 인수를 받는 방법에 대해 알아보겠습니다. 이 방법을 잘 배워두면 임의의 수의 인수나 요소를 받는 함수 혹은 배열을 쉽게 다룰 수 있게 됩니다.

## 나머지 매개변수 `...`

어떻게 정의되었는지 상관없이, 함수는 인수 개수의 제약 없이 호출 가능합니다.
...
인수 개수는 "초과"하였지만, 오류가 발생하지 않습니다. 하지만 당연히 처음 두 개의 인수만을 이용해 반환값이 계산되죠.
...
첫 번째 매개변수는 변수로, 나머지 매개변수는 배열로 모을 수도 있습니다.

아래에선 처음 두 인수는 변수에, 나머지는 `titles` 배열에 모았습니다.

```js run
function showName(firstName, lastName, ...titles) {
  alert( firstName + ' ' + lastName ); // Julius Caesar

  // 나머지는 배열 titles에 모입니다.
...
나머지 매개변수는 지원된 지 얼마 안 된 비교적 최신의 문법입니다. 과거엔 매개 변수의 숫자와 관계없이 모든 인수를 사용하려면 `arguments` 객체를 사용해야만 했었습니다.

물론 지금도 여전히 `arguments`를 사용할 수 있습니다.

그러나 `arguments`는 유사 배열이고 이터러블(iterable)이지만 배열은 아닙니다. 따라서 배열 메서드를 지원하지 않습니다. `arguments.map (...)` 등을 호출할 수 없죠.
...
앞서 배운 바와 같이 화살표 함수에는 자신만의 `this`가 없습니다. 위 예제를 통해 확인한 바와 같이 화살표 함수에는 `arguments` 객체 또한 없습니다.
...
예시를 들어 이런 경우를 알아보겠습니다. [Math.max](mdn:js/Math/max) 내장함수는 주어진 숫자 목록에서 가장 큰 숫자를 반환합니다.
...
이럴 때 *전개 연산자(spread operator)*를 사용할 수 있습니다. 전개 연산자는 `...`를 사용하기 때문에 나머지 매개변수와 비슷해 보이지만, 나머지 매개변수와 반대로 작동합니다.
