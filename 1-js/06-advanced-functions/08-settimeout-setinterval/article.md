...
브라우저는 대게 스크립트의 실행이 완료된 이후에 DOM "다시 그리기"를 수행합니다. 따라서 요소 변경이 많은 하나의 거대한 함수를 실행하는 경우에, 스크립트가 완료될 때까지 변경 사항은 화면에 그려지지 못합니다.
...
<script>
  ...
  function count() {
    for (let j = 0; j < 1e6; j++) {
      i++;
      // 현재 i 값을 <div> 에 넣습니다.
      // (innerHTML에 대해선 추후 관련 챕터에서 자세히 알아보도록 하겠습니다.)
