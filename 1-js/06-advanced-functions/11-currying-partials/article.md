...
아래는 `bind`를 어떻게 사용하는지를 나타내는 문법입니다.
...
이것을 [partial 함수 적용](https://en.wikipedia.org/wiki/Partial_application) -- 기존 매개변수의 일부 매개변수를 고정하여 새로운 함수를 작성한 것 이라고 합니다.

여기서 실제로 `this`를 사용하지 않는다는 것을 주의해야 합니다. 그러나 `bind` 할 대상이 필요로하기 때문에 `null`과 같은 것을 넣어야 하는 것이죠.
...
그 밖의 경우에는, parial 적용은 매우 제네릭한 함수나 부분적으로 다목적인 함수를 편리하게 사용하고 싶을때 사용합니다.
...
- 그다음에 `sum(1)(2)`이 마침내 `2`를 제공하는 `function(b)`를 호출합니다. 그리고 그것이 원래의 다중인수를 가진 `sum`으로 넘겨져서 호출됩니다. 
...
2. 오늘의 logs를 출력하는 것처럼 partial 함수를 생성할 수 있습니다.
...
2. Get a partial: 아니면, `func`이 아직 호출되지 않았습니다. 대신에 `pass`라는 다른 래퍼가 반환되고, 그것이 이전 인수들과 함께 새로운 인수들을 제공하는 `curried`에 다시 적용됩니다. 그다음에 새로운 호출에, 다시, 새로운 partial을 받거나 (만약에 인수들이 충분하지 않다면) 아니면 최종적으로 결과를 받습니다. 
...
그러나 커링의 구현은 자바스크립트에서 고급단계입니다. 이번 챕터에서 알아보았듯이 커링은 함수를 다중-인수 변형 형태로 호출 가능할 수 있도록 합니다. 
```
...
- 기존 함수의 일부 인수를 고정해서 (다목적이지 않은) 함수를 만들어내는것을 *a partial* 이라고 합니다.. `bind`를 사용하여 partial을 구현할 수 있지만 다른 방법들도 있습니다.
    ...
    Partials 은 똑같은 인수를 계속 사용해야 할 때 편리합니다. `send(from, to)` 같은 함수가 있고 `from`을 항상 같은 값으로 넘겨야 한다고 할때, partial을 적용해서 일괄적으로 적용할 수 있습니다.

- *커링*은 `f(a,b,c)`를 호출할 수 있는 `f(a)(b)(c)` 함수로 변환하는 기술입니다. 보통 자바스크립트에서의 구현은 해당 함수는 평소처럼 호출도 하고 만약에 인수들이 충분하지 않을 경우 partial을 반환합니다. 

    커링은 partials를 쉽게 할 수 있을 때 좋습니다. 로그 예시에서 보았듯이 다목적으로 쓰였던 `log(date, importance, message)` 함수는 커링후에 `log(date)`같이 하나의 인수를 가진 형태나 `log(date, importance)`처럼 두 개의 인수를 가진 형태로 호출할 수 있었습니다.
