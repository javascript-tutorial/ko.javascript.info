...
자바스크립트는 함수들을 대할 때 특별한 유동성을 제공합니다. 함수들이 서로 전달될 수 있고 객체처럼 사용될 수 있으며, 이번 챕터에서는 더 나아가 함수들이 어떻게 *포워드* 되고 *데코레이트(decorate)* 되는지 살펴보겠습니다.  

## 투명한 캐싱(Transparent caching)

`slow(x)`라는 CPU 사용률이 무겁지만, 결과는 안정적인 함수가 있다고 가정해보죠. 다르게 표현하면, 똑같은 `x`라는 값에 대해 항상 같은 결과를 반환한다고 해봅시다. 

만약에 이러한 함수가 자주 호출된다면, 아마도 다른 `x`의 결과들을 캐시(기억) 해서 다시 계산하는 데 소모되는 추가적인 시간을 피하고 싶을 것입니다.

그런데 이런 방법을 구현할 때 `slow()`에 추가적인 기능을 더하는 대신 래퍼(wrapper)를 생성할 수 있습니다. 아래에서 살펴볼 것처럼 이렇게 하는 데는 많은 이점이 있습니다.

아래에 코드와 설명이 있습니다.

```js run
function slow(x) {
  // 아래 작업은 CPU를 많이 사용합니다. 
  alert(`Called with ${x}`);
  return x;
}
...
위의 예시에서 `cachingDecorator`코드가 바로 *데코레이터* 입니다: 또 다른 함수를 가져와서 그 함수의 행동을 변환하는 특별한 함수

중요한 것은 `cachingDecorator`는 대상이 어떤 함수이던지 상관없이 호출할 수 있고 그 함수는 캐싱 래퍼를 반환할 것입니다. 이런 것은 좋습니다. 왜냐하면 어떠한 함수들도 특별한 기능처럼 사용할 수 만들기 때문이죠, 그리고 오직 필요한 것은 `cachingDecorator`를 적용하는 것뿐입니다.

caching이 해당하는 메인 함수 코드로 부터 분리된 것은 메인 코드를 간단하게 하는 데도 도움이 됩니다.

이제, 어떻게 작동하는지 자세히 알아보겠습니다.

`cachingDecorator(func)`의 결과값은 "래퍼" 입니다: `function(x)` 이라는 함수로 "둘러싼" `func(x)`의 호출을 caching 로직으로 담아냅니다:
...
요약하면, `slow` 코드를 직접 수정하는 것보다 독립적인 `cachingDecorator`를 사용하는 것은 다음과 같은 많은 이점이 있습니다:
...
그렇기 때문에 래퍼는 컨텍스트에 있는 `this` 없이 원래 가지고 있던 메서드를 호출하기 때문에 에러가 발생합니다.
...
자세히 이해하기 위해서 `this`가 어떻게 전달되었는지 자세히 알아보겠습니다:
...
3. wrapper 안에서는, 결과가 캐시되지 않았다고 가정합니다, `func.call(this, x)`이 현재의 `this`(`=worker`)를 전달하고 현재의 인수 (`=2`)를 원래 매서드에 전달합니다.
...
첫 번째는 어떻게 `min` 과 `max`두 인수를 `cache` 맵의 키로 사용할 것인가 입니다. 한가지 인수를 사용할 때 `x`는 `cache.set(x, result)`을 사용해서 결과를 저장하고 `cache.get(x)`을 사용하면 되었습니다. 그러나 이제 *인수들의 조합* `(min,max)`를 기억해야 합니다. 네이티브 `Map`은 오직 한가지 값을 키로 가질 수 있습니다.
...
대다수의 애플리케이션들은 3번째 방법이 적절합니다. 여기에서도 그 방법을 선택하도록 하겠습니다.

두 번째 과제는 많은 인수를 어떻게 `func`으로 전달할까 하는 것입니다. 아직은 `function(x)` 래퍼는 하나의 인수를 가정하고 그것을 `func.call(this, x)`로 전달합니다.

이럴 때 [func.apply](mdn:js/Function/apply)라는 내장함수를 사용할 수 있습니다.
...
위의 함수는 `func`의 `this=context`를 설정하고 `args` 배열 같은 객체를 사용해서 인수들을 리스트로 합니다.
...
자세히 보면 `call`과 `apply` 사이에는 조금 다른 점이 있습니다.
...
그리고 `args`가 iterable 하면서 유사 배열일 경우에는, 기술적으로 실제 배열이면 어떤 것이든 사용할 수 있습니다만, 아마도 `apply`가 단일 작업이기 때문에 더 빠르게 동작할 것 입니다. 대부분 자바스크립트 엔진은 내부적으로 `call + spread`를 함께 쓰는 것보다 최적화가 잘 되게 되어있기 때문이죠.
...
위와 같은 것을 *call forwarding*이라고 합니다. `래퍼`가 받는 모든 것을 전달합니다: `this`라는 컨텍스트와 인수들을 `anotherFunction`의 결과와 함께 반환합니다.

외부의 `래퍼` 함수가 호출되었을 때, 원래 함수의 호출로부터 구별할 수 없습니다.
...
위의 예시는 아직까진 두 개의 인수들로만 작동합니다. 이것을 몇 개의 `args`이던지 상관없이 작동하게 된다면 더 좋을 것 같습니다.

자연스러운 해결법은 [arr.join](mdn:js/Array/join) 메서드를 사용하는 것 입니다.
...
...안타깝게도, 위의 예시는 작동하지 않습니다. 왜냐하면 `hash(arguments)`와 `arguments` 객체 모두 정규 배열은 아니면서 iterable 이고 유사 배열이기 때문이죠, 
...
일반적으로 하나의 함수나 하나의 메서드를 하나로 데코레이트 하는 것이 안전합니다. 원래 함수가 `func.calledCount` 또는 어떤 것이든, 이와 같은 속성을 가지고 있다면, 그것은 래퍼이기 때문에 데코레이트 된 후의 것은 프로퍼티를 가지고 있지 않을 것입니다. 데코레이터는 자신만의 프로퍼티를 제공하기 때문에 데코레이터를 사용할 때는 신중해야 합니다.

데코레이터들은 한 개 또는 여러 개의 "기능들(features)" 또는 "관점들(aspects)"을 원래 함수를 변경하지 않고! 주입할 수 있습니다. 

`cachingDecorator`를 구현하기 위해서는 다음과 같은 메서드를 공부했습니다
