...
그러나 좋은 코드엔 "설명이 담긴(explanatory)" 주석이 많아선 안 됩니다. 주석 없이도 코드 자체만으로 코드가 무슨 일을 하는지 쉽게 이해할 수 있어야 합니다.
...
알고리즘이 복잡한 경우, 코드 자체만으로도 어떤 일을 하는지 충분히 판단할 수 있게(자기 설명적) 작성할 수 없는 특수한 상황이라면 주석을 넣으세요.
