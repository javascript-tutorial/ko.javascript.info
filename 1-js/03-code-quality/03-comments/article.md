...
주석을 작성하는 게 처음엔 쉬워 보일 수 있겠지만, 초보 프로그래머들은 종종 주석을 잘못된 방법으로 작성합니다.
...
그러나 좋은 코드에선 "설명적(explanatory)" 주석이 최소화되어야 합니다. 주석이 없어도 코드 자체만으로 코드가 무엇을 하는지 쉽게 이해할 수 있어야 합니다.
...
아키텍처를 설명하는 주석
: 고차원 수준의 컴포넌트 개요, 컴포넌트 간의 상호작용에 대한 설명, 상황에 따른 제어 흐름 등을 설명하는 주석이 좋습니다. 이런 주석은 코드의 조감도 역할을 합니다. 고차원 수준의 아키텍처 다이어그램을 그리는 데 쓰이는 언어인 [UML](http://wikipedia.org/wiki/Unified_Modeling_Language)이라는 게 있는데, 시간을 내어 공부해 보시기 바랍니다.

함수의 용례와 매개변수에 대한 주석
: 함수의 용례, 매개변수, 반환값등을 문서로 만드는 데 쓰이는 [JSDoc](http://en.wikipedia.org/wiki/JSDoc)이라는 특별한 문법이 있습니다.
