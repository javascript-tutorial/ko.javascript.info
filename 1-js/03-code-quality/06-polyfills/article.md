...
2. 폴리필
    ...
    이렇게 기존 함수의 동작 방식을 수정하거나, 새로운 표준에 등록된 정의를 준수할 수 있게 구현한 함수에 관한 스크립트를 "폴리필(polyfill)"이라 부릅니다. 폴리필(poly`fill`)은 구현이 누락된 새로운 기능을 메꿔주는(`fill in`) 역할을 합니다.

    주목할 만한 폴리필 두 가지를 소개해 드리겠습니다.
...
모던 자바스크립트 기능을 사용하려면 트랜스파일러와 폴리필은 필수입니다.
...
Google Chrome은 모든 브라우저 중 대게 가장 먼저 최신 기능을 지원합니다. 따라서 크롬을 사용하면 트랜스파일러 없이 최신 기능을 이용해 데모하기 용이합니다. 다른 브라우저들도 많이 뒤처지는 편은 아니니 안심하고 사용하셔도 됩니다.
