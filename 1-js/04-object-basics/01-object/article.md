...
서랍장(객체 `user`) 안에 파일 두 개(프로퍼티 두 개)가 담겨있는데, 각 파일에 "name", "age"라는 이름표가 붙어있다고 생각하면 쉽습니다.
...
점 표기법은 키가 '유효한 변수 식별자'인 경우에만 사용할 수 있습니다. 유효한 변수 식별자가 되려면 공백이 없어야 합니다.

키가 유효한 변수 식별자가 아닌 경우엔 점 표기법 대신 '대괄호 표기법(square bracket notation)'을 사용할 수 있습니다. 대괄호 표기법은 키에 어떤 문자열이 있든지 상관없이 동작합니다.
...
이제 문법 에러가 발행하지 않네요. 대괄호 표기법을 사용할 때는 대괄호 안 문자열을 따옴표로 묶어줘야 합니다. 따옴표의 종류(큰따옴표, 작은따옴표)는 상관없습니다.

대괄호 표기법을 사용하면 아래 예시에서 변수를 키로 사용한 것과 같이 문자열뿐만 아니라 모든 표현식의 결과를 프로퍼티 키로 사용할 수 있습니다.
...
변수 `key`는 런타임에 계산되거나 사용자 입력에 따라 그 값이 변경될 수 있습니다. 어떤 경우든, 최종 결괏값(계산 결과나 변경된 값)이 프로퍼티 키로 사용됩니다. 이를 응용하면 코드를 유연하게 작성할 수 있습니다.
...
그런데 점 표기법은 아래 예시와 같은 방식으로 사용할 수 없습니다.
...
그래서 프로퍼티 이름이 정해져 있고 간단할 경우엔 주로 점 표기법을 사용합니다. 그러다 더 복잡한 이름이 필요할 때 대괄호 표기법을 사용하죠.
...
객체 프로퍼티 이름엔 제약이 없지만 역사적인 이유 때문에 특별 대우를 받는 이름이 하나 있습니다. 바로 `__proto__`입니다. 프로퍼티 이름(키)이 `__proto__`인 경우엔 값에 객체만 올 수 있습니다. 객체 이외의 자료형은 프로퍼티 값으로 설정할 수 없습니다.
...
객체의 중요한 특징 중 하나는 존재하지 않는 프로퍼티에 접근하면 에러가 발생하지 않고 `undefined`를 반환한다는 것입니다. 이런 특징을 응용하면 프로퍼티 존재 여부를 확인할 수 있습니다. 
...
`for..in` 반복문에서도 `for(;;)`문처럼 반복 변수(looping variable)를 선언했다는 점에 주목하길 바랍니다.
...
나라 번호(키)가 정수이어서 `1, 41, 44, 49` 순으로 프로퍼티가 자동 정렬되었기 때문이죠.
...
두 변수가 같은 객체를 참조하는 예시를 살펴봅시다. 일치/동등 비교 시 참이 반환됩니다.
...
다른 예시를 살펴봅시다. 두 객체 모두 비어있다는 점에서 같아 보이지만, 독립된 객체이기 때문에 일치/동등 비교 시 거짓이 반환됩니다.
...
그러나 정말 복제가 필요한 상황이라면 새로운 객체를 만든 다음 기존 객체의 프로퍼티들을 순회해 원시 수준까지 프로퍼티를 복사해 객체를 복제하면 됩니다.
...
복사된 프로퍼티를 받는 객체(`user`)에 동일한 이름을 가진 프로퍼티가 있으면 복사된 프로퍼티의 값을 덮어씁니다.
...
깊은 복사 시 사용되는 표준 알고리즘인 [Structured cloning algorithm](https://html.spec.whatwg.org/multipage/structured-data.html#safe-passing-of-structured-data)을 사용하면 위 사례를 비롯한 다양한 상황에서 객체를 복제할 수 있습니다. 자바스크립트 라이브러리 [lodash](https://lodash.com)의 메서드인 [_.cloneDeep(obj)](https://lodash.com/docs#cloneDeep)을 사용하면 이 알고리즘을 직접 구현하지 않고도 깊은 복사를 처리할 수 있으므로 참고하시기 바랍니다.
