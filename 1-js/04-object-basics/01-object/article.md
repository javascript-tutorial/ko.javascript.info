...
점은 키가 '유효한 변수 식별자'인 경우에만 사용할 수 있습니다. 유효한 변수 식별자엔 공백이 없어야 합니다. 또한 숫자로 시작하지 않아야 하며 `$`와 `_`를 제외한 특수 문자가 없어야 합니다.
...
이제 문법 에러가 발행하지 않네요. 대괄호 표기법 안에서 문자열을 사용할 땐 문자열을 따옴표로 묶어줘야 한다는 점에 주의하시기 바랍니다. 따옴표의 종류는 상관없습니다.
...
프로퍼티 이름(키)이 `__proto__`인 경우엔 값에 객체만 올 수 있습니다. 객체 이외의 자료는 프로퍼티 값으로 설정할 수 없습니다.
...
위 예시에서 원시값 `5`를 할당했는데 무시되었습니다.

`__proto__`의 본질은 [](info:prototype-inheritance)에서 자세히 다룰 예정입니다.

여기선 `__proto__` 이런 특징이 버그나 취약성의 원인이 될 수 있기 때문에 잘 알아두어야 한다는 것 정도만 숙지하면 될 것 같 좋습니다.
 사용자가 키를 지정할 수 있도록 코드를 짜면 예상치 못한 일이 일어날 수 있습니다.
 
사이트 방문자가 `__proto__`를 키로 사용하면, 위 예시에서처럼 할당 관련 로직이 무시될 가능성이 있기 때문입니다.

이런 문제를 피할 방법은 두 가지가 있습니다.
1. `__proto__`도 일반 프로퍼티처럼 동작할 수 있게 객체의 행동을 바꾸는 것입니다. 자세한 방법에 대해선 [](info:prototype-methods)에서 다루겠습니다.
