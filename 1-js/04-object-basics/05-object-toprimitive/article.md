...
객체 형 변환은 세 종류로 구분되는데, "hint"라 불리는 값이 구분 기준이 됩니다. hint가 무엇인지는 [명세](https://tc39.github.io/ecma262/#sec-toprimitive)에 자세히 설명되어 있는데, "목표로 하는 자료형" 정도로 이해하시면 될 것 같습니다. 

`"string"`
: 함수 `alert`같이 문자열을 기대하는 연산을 수행할 때(객체-문자형 변환), hint가 `string`이 됩니다.
...
: 연산자가 기대하는 자료형이 '확실치 않을 때', hint는 `default`가 됩니다. 아주 드물게 발생합니다.
    ...
    참고로, 크고 작음을 비교할 때 쓰이는 연산자 `<`, `>` 역시 피연산자에 문자형과 숫자형 둘 다를 허용하는데, 이 연산자들은 hint를 "number"로 고정하게 합니다. hint가 "default"가 되는 일이 없죠. 이는 하위 호환성 때문에 정해진 규칙입니다.

    `Date` 객체를 제외한 모든 내장 객체는 hint가 `"default"`인 경우와 `"number"`인 경우를 동일하게 처리합니다. 우리도 커스텀 객체를 만들 땐 이런 규칙을 따르는 게 좋겠죠?

hint는 총 세 가지입니다. 아주 간단하죠. hint가 "boolean"인 경우는 존재하지 않습니다. 모든 객체는 그냥 `true`로 평가될 뿐입니다. 게다가 내장 객체처럼 `"default"`와 `"number"`를 동일하게 처리하면, 결국엔 두 종류의 형 변환(객체-문자형, 객체-숫자형)만 남게 됩니다.

**자바스크립트는 형 변환이 필요할 때, 아래와 같은 알고리즘에 따라 원하는 메서드를 찾고 호출합니다.**

1. 객체에 `obj[Symbol.toPrimitive](hint)`메서드가 있는지 찾고, 있다면 메서드를 호출합니다. `Symbol.toPrimitive`는 시스템 심볼이며, 심볼형 키로 사용됩니다.
