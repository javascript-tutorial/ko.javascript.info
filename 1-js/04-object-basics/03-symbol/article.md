...
설명(심볼 이름이라 부름)을 붙여 심볼을 만들 수도 있습니다. 심볼 이름은 디버깅 시 유용하게 쓰입니다.
...
이름이 같은 심볼이라도, 값은 항상 다릅니다. 이름이 같은 심볼이 같은 값을 갖길 원한다면, 전역 레지스트리를 사용해야 합니다. `Symbol.for(key)`는 `key`라는 이름을 가진 전역 심볼을 반환합니다(심볼이 없다면 생성해줍니다). 동일한 `key`로 Symbol.for`를 호출하면 항상 같은 심볼을 반환합니다.
