...
자바스크립트의 메모리 관리는 자동으로 수행되며 우리 눈에는 보이지 않습니다. 우리가 만드는 기본값이 할당된 변수, 객체, 함수... 이 모든 것은 메모리를 차지하고 있습니다.

만약 어떤 것이 더 필요하지 않을 땐 무슨 일이 일어날까요? 자바스크립트 엔진은 그것을 어떻게 찾아내서 삭제하는 걸까요?

## 도달 가능성(Reachability)

자바스크립트에서 메모리 관리의 주요 개념은 *도달 가능성(Reachability)*입니다.

쉽게 말해, "도달 가능한" 값들은 접근할 수 있으며 어떻게든 사용 가능한 값들을 말합니다. 이 값들은 메모리에 저장되는 게 보장됩니다.

1. 본질적으로 도달 가능한 값들의 기본 집합이 있습니다. 이 값들은 어떤 이유로도 삭제될 수 없습니다.
...
2. 루트에서 참조나 참조의 연쇄에 의해 도달 가능한 값이 있다면 그 값은 도달 가능합니다.
...
자바스크립트 엔진에는 [가비지 컬렉터(garbage collector)](https://en.wikipedia.org/wiki/Garbage_collection_(computer_science)) 라는 백그라운드 프로세스가 있습니다. 가비지 컬렉터는 모든 객체를 감시하면서 도달할 수 없게 된 객체를 삭제합니다.
...
만약 `user` 값이 다른 값으로 덮어 쓰이면 참조를 잃게 됩니다.:
...
이제 John은 도달할 수 없게 됩니다. John에 접근할 방법은 없으며, John에 대한 참조도 존재하지 않습니다. 가비지 컬렉터는 이 데이터를 삭제하고 메모리를 해제할 것입니다.
...
`marry` 함수는 두 객체에 서로에 대한 참조를 부여하며 두 객체를 "결혼"시킵니다. 이 함수는 두 객체를 포함한 새로운 객체를 반환합니다.
...
모든 객체는 아직 접근할 수 있기 때문에 참조 하나를 지우는 것만으로는 충분하지 않습니다.

그러나 두 참조를 모두 지우면, John은 더 이상 외부에서 들어오는 참조를 갖지 않게 됩니다.:

![](family-no-father.png)

외부로 나가는 참조는 문제 되지 않습니다. 외부에서 들어오는 참조만 객체를 도달 가능한 상태로 만듭니다. 그러므로 John은 이제 도달할 수 없으며, 접근할 수 없게 된 John의 데이터와 함께 메모리에서 제거될 것입니다.
...
서로 연결된 객체들의 섬 전역이 도달할 수 없게 돼 메모리에서 제거될 수도 있습니다.
...
John과 Ann은 여전히 연결되어 있으며 둘 다 외부에서 들어오는 참조를 갖고 있다는 사실이 명백합니다. 하지만 이걸로 충분하지 않습니다.

앞서 `"family"` 객체가 루트로부터 연결이 해제되었으므로, 이제 이 객체에 대한 참조는 존재하지 않으며 섬 전체가 도달할 수 없게 돼 메모리에서 제거될 것입니다.
...
- 그리고 루트가 참조하는 모든 것들에 방문하여 "마크" 합니다.
- 이제 마크된 모든 객체에 방문하여 *그들의* 참조들에 마크합니다. 같은 객체를 나중에 다시 방문하는 것을 방지하기 위해 모든 방문된 객체들은 기억됩니다.
...
오른쪽에 "도달할 수 없는 섬" 을 분명히 볼 수 있습니다. Now let's see how "mark-and-sweep" garbage collector deals with it.
