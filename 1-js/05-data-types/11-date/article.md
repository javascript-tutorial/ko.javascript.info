...
예를 들어 시각을 생성/수정한 후 저장하거나, 시각을 측정하거나, 현재 날짜를 출력하기 위해 Date 객체를 사용할 수 있습니다.
...
: 1970년 1월 1일(UTC+0)을 기준으로, 인자로 주어진 밀리초(1/1000초)만큼 지난 날짜에 대한 `Date` 객체를 만듭니다.
    ...
    // 24시간을 더하자 1970년 1월 2일(UTC)이라는 값을 얻었습니다.
    let Jan02_1970 = new Date(24 * 3600 * 1000);
    alert( Jan02_1970 );
    ```

    1970년 1월 1일을 기준으로 몇 밀리초를 지났는지 나타내는 정수 숫자를 *timestamp*라고 부릅니다.

    *timestamp*는 날짜를 나타내는 가벼운 숫자 표현입니다. 우리는 `new Date(timestamp)`를 사용해 *timestamp*로 date 객체를 만들 수 있고, `date.getTime()`메소드를 이용해 이미 존재하는 `Date` 객체를 *timestamp*로 바꿀 수 있습니다. 

`new Date(datestring)`
: 만약 문자열로 되어있는 하나의 인자가 있다면, 인자는 자동으로 분석됩니다. 여기에 쓰이는 알고리즘은 `Date.parse`를 사용하는 것과 같은데, 이는 나중에 다룰 것입니다.
...
: 주어진 구성요소들의 지역 시간대로 date 객체를 생성합니다. 맨 앞의 두 인자는 필수적으로 들어가야 합니다.

    - `year` 는 숫자 4개여야 합니다. `2013`은 괜찮지만 `98`은 그렇지 않습니다.
    - `month`의 셈은 `0`(1월) 부터 시작해서 `11`(12월)로 끝납니다.
    - `date` 매개변수는 "day"을 의미하며 공백으로 남긴다면 1로 추정됩니다.
    ...
    new Date(2011, 0, 1); // 인자에 쓰여져 있지 않더라도 각 인자들의 기본값이 0이기 때문에 결과는 같습니다.
    ```
...
```warn header = "`getYear()`가 아니라 `getFullYear()`"
많은 자바스크립트 엔진은 비표준 메소드인 `getYear()`을 실행합니다. 이 메소드는 2자리 숫자를 반환(return)하기 때문에 사용되지 않습니다. 절대 사용하지 말아 주세요. 연도를 위해 `getFullYear()`메소드가 있습니다.
```

추가로, 우리는 요일을 얻을 수 있습니다.

[getDay()](mdn:js/Date/getDay)
: 0(일요일)부터 6(토요일)까지 요일을 가져옵니다. 첫 번째 요일은 일요일부터 시작합니다. 몇몇 나라에서는 그렇지 않지만, 이는 바뀌지 않습니다.

**위의 모든 메소드는 지역 시간대에 따라 상대적인 구성요소를 반환합니다.**
...
지역 시간대가 UTC에 상대적으로 바뀌면 아래 코드는 다른 시간을 보여줍니다. 
...
// UTC+0 시간대에서 시간 (일광 절약이 없는 런던 시각)
...
: 해당 날짜의 타임스탬프 (1970년 1월 1일 UTC+0에서 지난 밀리초의 숫자) 를 반환합니다.

[getTimezoneOffset()](mdn:js/Date/getTimezoneOffset)
: 지역 시간대와 UTC의 차이를 분으로 반환합니다.
...
다음 메소드들은 날짜/시각 컴포넌트들을 설정할 수 있게 해줍니다.
...
- [`setTime(milliseconds)`](mdn:js/Date/setTime) (1970년 1월 1일(UTC) 이후 밀리초로 전체 날짜를 설정합니다).

위의 메소드 중 `setTime()`를 제외한 메소드들은 `setUTCHours()`와 같은 UTC-버전 메소드가 있습니다.

보다시피 `setHours`와 같은 메소드들은 한 번에 여러 구성요소들을 설정할 수 있습니다. 언급되지 않은 구성요소들은 수정되지 않습니다. 
...
*Autocorrelation*은 `Date` 객체의 아주 유용한 특성입니다. 우리는 허용 범위 밖의 값을 설정할 수 있고, 그것은 스스로 교정될 것입니다.
...
"2016년 2월 28일"에서 2일을 더해야 한다고 해봅시다. 그 결과는 윤년인지에 따라 "3월 2일" 혹은 "3월 1일" 일 것입니다. 우리는 윤년의 여부에 대해 생각하지 않고 그냥 2일을 더하면 됩니다. `Date` 객체가 나머지 일들을 할 것입니다.
...
`Date` 객체가 숫자로 바뀔 때, *timestamp*가 `date.getTime()` 메소드와 같게 됩니다.:

```js run
let date = new Date();
alert(+date); // date.getTime()의 결과와 같은 밀리초 단위 숫자
```

중요한 부가 효과: 날짜를 뺄 수 있고 그 결과는 밀리초 단위로 된 두 날짜의 차이입니다.

이는 시간 측정에 활용될 수 있습니다.
...
현재 *timestamp*를 반환해주는 `Date.now()`라는 특별한 메소드가 있습니다.

의미적으로는 `new Date().getTime()`과 같지만 `Date.now()` 메소드는 중간 `Date` 객체를 만들지 않습니다. 그래서 더 빠르고 가비지 컬렉션에 부담을 주지 않습니다.

그렇기 때문에 자바스크립트 게임이나 다른 특별한 애플리케이션처럼 편의성이나 성능이 중요할 때 자주 사용합니다.

이런 이유로 `Date.now()`메소드가 더 좋을지도 모릅니다.
...
CPU를 많이 사용하는 함수의 벤치마킹을 시행할 때 주의해야 합니다.
...
```js
// 우리는 date1과 date2를 가지고 있습니다. 어느 함수가 더 빠르게 밀리초로 그 차이를 반환할까요?
...
첫 번째 아이디어는 그것들을 연속으로 여러 번 실행하고 시간의 차이를 측정하는 것일지도 모릅니다. 우리의 경우, 함수는 아주 간단하기 때문에 우리는 최소한 이 일을 10만 번 정도 해야 합니다.
...
`bench(diffSubtract)` 함수를 실행할 때, CPU가 자원을 사용하는 어떤 작업을 병렬적으로 수행하고 있다고 상상해 봅시다. `bench(diffGetTime)` 함수가 실행될 때 그 작업이 끝납니다.

현대의 멀티 프로세스 운영체제에서 자주 볼 수 있는 상황입니다.

결과적으로 첫 번째 벤치마크는 두 번째 벤치마크보다 더 적은 CPU 자원을 가질 것입니다. 이는 잘못된 결과를 가져올지도 모릅니다. 

**더 신뢰할 수 있는 벤치마킹을 위해 벤치마크 전체 패키지는 여러 번 다시 실행되어야 합니다.**
...
모던 자바스크립트 엔진은 여러 번 실행되는 "핫 코드(hot code)"에만 고급 최적화를 적용하기 시작했습니다. 잘 실행되지 않는 것들에는 최적화가 크게 의미있지 않기 때문입니다. 위의 예에서, 함수가 처음 실행 될 때는 최적화된 상황이 아닙니다. 같은 상황에서 두 함수를 비교하기 위해, 열을 가하는 첫 실행 (heat-up run)을 더해 함수들이 최적화된 환경을 만들어야 할 것입니다.
...
```warn header="벤치마킹을 수행할 때 주의하십시오."
