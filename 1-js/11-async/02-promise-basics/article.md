...
당신은 일의 부하를 덜기 위해 앨범이 출시되면 팬들이 자동으로 소식을 받아볼 수 있도록 해줄 겁니다. 구독 리스트를 하나 만들어 팬들에게 이를 전달하며, 여기에 자신의 이메일 주소를 적게 만들겠죠. 앨범이 준비되면 약속한 대로 구독 리스트에 자신의 메일을 적은 팬들은 즉시 소식을 받아볼 수 있을 겁니다. 출시 예정인 앨범이 취소되는 불상사가 발생해도 소식 전할 수 있죠.

이제 모두가 행복해졌네요. 밤낮으로 질문을 하는 팬들이 사라졌고, 팬들은 앨범 출시를 놓치지 않을 수 있게 되었으니까요.
...
2. "소비 코드(consuming code)"는 "제작 코드"가 준비되었을 때, 제작 코드의 결과를 사용하는 코드입니다. 복수의 함수(소비 코드)에서 제작 코드의 결과를 활용하고 싶어 할 수 있습니다. 위 비유에선 제작 코드의 결과물을 기다리는 함수를 "팬"으로 비유했습니다.
3. *프라미스(promise)* 는 "제작 코드"와 "소비 코드"를 연결해 주는 특별한 자바스크립트 객체입니다. 위 비유에서 "구독 리스트"가 프라미스입니다. "제작 코드"는 시간이 얼마나 걸리든 상관없이 약속한 결과물을 만들어 냅니다. "프라미스"는 결과물이 준비되었을 때, 모든 구독 코드에서 이 결과를 사용할 수 있도록 해줍니다.

자바스크립트 프라미스는 비유에 사용된 구독 리스트보다 훨씬 복잡하기 때문에, 위 비유가 완벽하게 들어맞지는 않습니다. 프라미스엔 비유에서 설명하지 못한 추가적인 기능과 한계점이 있습니다. 하지만 일단 이 비유를 이용해 프라미스를 학습해보도록 합시다.
...
일단, 간단한 executor를 이용해 만든 프라미스 생성자를 살펴보도록 합시다. 이 함수는 `setTimeout`을 이용해 만든 "제작 코드"로 구성하였습니다. 
...
executor는 `resolve`나 `reject` 함수 중 하나를 반드시 호출해야 합니다. 함수가 호출되어 프라미스의 state가 변경되면, state는 더이상 변하지 않습니다.
...
어떤 일을 시작했는데, 알고 보니 일이 이미 끝나 저장까지 되어있는 경우가 이렇게 처리할 수 있겠죠.

이 경우, 이행된(resolved) 프라미스를 즉시 갖게 됩니다.
````

```smart header="`state`와 `result`는 내부에 있습니다."
프라미스 객체의 `state`, `result` 프로퍼티는 내부에 있어서 "소비 코드"에선 이 프로퍼티에 바로 접근할 수 없습니다. `.then`/`.catch`/`.finally` 메서드를 사용하면 이 프로퍼티에 접근할 수 있습니다. 아래에서 이 메서드에 대해 살펴보도록 하겠습니다.
```

## 소비자: then, catch, finally

프라미스 객체는 executor("제작 코드" 혹은 "가수")와 결과나 에러를 받을 소비 함수("팬")를 이어주는 일을 합니다. 소비 함수들은 `.then`, `.catch`, `.finally` 메서드로 등록(구독)됩니다.
...
다음은 프라미스가 성공적으로 이행된 경우에 관한 예시입니다.
...
프라미스가 처리되면 항상 실행된다는 점에서 `.finally(f)` 호출은 `.then(f, f)`과 유사합니다. 프라미스가 이행(resolved)이나 거부(rejected) 일 때 실행되죠.
...
1. `finally` 핸들러엔 인수가 없고, 프라미스가 이행되었는지, 거부되었는지 여부도 알 수 없습니다. finally 에선 절차를 마무리 하는 "보편적" 동작을 하기 때문에, 성공/실패 여부를 몰라도 됩니다.
2. Finally는 결과(result)와 에러(error)를 다음 핸들러에게 전달합니다.
    ...
    finally는 프라미스의 결과를 처리하려는 목적으로 만들어지지 않았기 때문에, 이행 여부에 상관없이 결과를 전달해줍니다. 이런 특징은 유용하게 사용될 수 있습니다.

    프라미스 체이닝과 핸들러 간 결과 전달에 대한 이야기는 다음 챕터에서 이어나가도록 하겠습니다.

3. `.finally(f)`는 `.then(f, f)`보다 문법 측면에서 더 편리합니다. 함수를 중복해서 사용할 필요가 없습니다.

````smart header="프라미스가 처리되면, 핸들러는 즉각 실행됩니다."
프라미스가 대기(pending)상태일 때, `.then/catch/finally` 핸들러는 결과를 기다립니다. 만약 프라미스가 처리되었다면(settled), 핸들러가 즉각 실행됩니다.
...
`.then` 핸들러는 시간이 걸리는 프라미스이든, 즉시 끝나는 프라미스이든 상관없이 실행이 보장됩니다.
````

이제, 프라미스를 이용해 어떻게 비동기 코드를 작성하는지 알아봅시다. 실질적인 예제와 함께 말이죠.
...
새로운 `loadScript` 함수엔 콜백 함수가 필요하지 않습니다. 대신, 스크립트가 완전히 불러졌을 때 이행되는 프라미스 객체를 만들고, 이를 반환해 줍니다. 외부 코드는 `.then`을 이용해 이 함수에서 반환된 프라미스에 핸들러(구독 함수)를 더할 수 있습니다.
...
| 프라미스 | 콜백 |
|----------|-----------|
| 프라미스를 이용하면 흐름을 자연스럽습니다. `loadScript(script)`로 스크립트를 읽고, 결과에 따라 그다음(`.then`)에 무엇을 할지에 대한 코드를 작성하면 되죠. | `loadScript(script, callback)`를 호출할 때, 함께 호출할 `callback` 함수가 준비되어 있어야 합니다. `loadScript`를 호출하기 *이전*에 호출 결과로 무엇을 할지 미리 알고 있어야 하죠.|
