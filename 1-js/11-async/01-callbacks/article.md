...
아래와 같이 함수를 호출해 스크립트를 불러올 수 있습니다.
...
함수를 호출하면 스크립트 로딩이 시작됩니다. 실행은 로딩이 끝난 후 진행됩니다. 스크립트가 로딩되는 동안 그 아래쪽 코드는 이미 실행이 끝났을 수 있습니다. 스크립트를 로딩하는 시간이 오래 걸리면, 그동안 다른 스크립트들도 실행될 수 있습니다.

```js
loadScript('/my/script.js');
// loadScript 함수 호출 코드의 아래쪽 코드는 스크립트 로딩이 끝날 때까지 기다리지 않고 실행됩니다.
// ...
```

스크립트 로딩이 끝나면, 내부의 스크립트를 사용하고 싶을 겁니다. 스크립트 안에는 다양한 함수가 선언되어있어, 외부에서 호출할 수 있습니다.  

하지만 만약 `loadScript(...)` 호출 직후에 (스크립트 로딩이 끝나길 기다리지 않고) 내부의 함수를 바로 호출하면, 원하는 대로 작동하지 않을 겁니다.
...
스크립트 안에 정의된 함수를 호출하고 싶다면, 그 함수를 콜백 함수 안에 써주면 됩니다:
...
두 번째 인수로 전달된 함수(대게 익명 함수)는 동작이 완료되었을 때 실행됩니다.
