...
위 예시의 `User`엔 메서드 하나만 있는데, 여기에 프로퍼티를 더해줘 봅시다.
...
`User.prototype`을 통해 프로퍼티 `name`을 정의하지 않고, `new`가 호출될 때, 생성자 메서드가 호출되기 전에 `name`이 만들어지도록 해보았습니다. 이렇게 만든 프로퍼티는 해당 객체만의 프로퍼티가 됩니다.
