...
그런 다음 `super` 는 그것을 사용하여 부모 프로토타입과 그 메소드를 결정합니다.
...
`[[HomeObject]]`는 클래스와 일반 객체 모두에서 메서드에 대해 정의됩니다. 그러나 객체의 경우, 메서드는 `method ()` 처럼 정확히 지정되어야 합니다 `"메서드: function () "`.

그 차이는 필수적이지는 않지만, 자바스크립트에서는 중요합니다.
...
4. Internals
    - 메서드는 내부 `[[HomeObject]]` 프로퍼티에 그것의 클래스와 오브젝트를 기억한다. 이것이 어떻게 `super`가 부모 메서들의 문제를 해결하는 방법이다.
    - 그래서 메서드를 `super`와 함께 한 객체에서 다른 객체로 복사하는 것은 안전하지 않다.

또한,
- 화살표 함수들은 자신의 `this`나 `super`를 가지고 있지 않다. 그래서 주변 컨텍스트에 투명하게 사용할 수 있다.
