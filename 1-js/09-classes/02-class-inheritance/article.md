...
그런데 `Rabbit`을 `Animal`의 연장선에 놓고 싶다고 한다면. 다르게 표현해서, rabbits는 animals를 기반으로 두고 있고, `Animal`의 메서드들과 독립적인 메서드들로 구성되게 하고 싶다고 해보죠.

다른 클래스로부터 상속받으려면, `"extends"`를 사용해서 braces `{..}`전에 부모 클래스를 명시해야 합니다.

아래코드는 `Rabbit`은 `Animal`을 상속하는 방법을 보여주고 있습니다.
...
이런 방법은 함수를 사용하여 다양한 조건에 따라 클래스를 생성하고 상속해야 하는 고급적인 프로그래밍 패턴을 사용할 때 유용합니다.
````
...
만약에 부모 함수에 접근 가능하다면, 그것은 외부의 함수에서 가져온 것일 것입니다. 아래 예제를 보면,
```js
class Rabbit extends Animal {
  stop() {
...
[자바스크립트 규격](https://tc39.github.io/ecma262/#sec-runtime-semantics-classdefinitionevaluation)에 따르면, 만약 클래스가 다른 클래스를 확장하고 그것이 `생성자`를 가지고 있지 않다면, "비어있는" `생성자`를 생성합니다.
...
간단히 설명하면, 상속받은 클래스들 안의 생성자는 `this`를 사용하기 전에 `super(...)`를 반드시 호출 해야 합니다.
...
작업은 단순해 보일 수도 있지만 그렇지 않습니다. 엔진은 현재 객체 `this`를 알고 있음으로, 부모 `method` 를 `this .__ proto __. method`로 얻을 수 있습니다만, 불행히도 이러한 "간단한" 해결책은 효과가 없습니다.
...
`tree.sayHi()`를 호출하면 "I'm an animal"을 출력합니다. 확실히 잘못되었습니다.
