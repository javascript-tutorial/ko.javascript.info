...
쿠키는 주로 웹 서버의 요청으로 생성됩니다.

인증은 쿠키의 주요 사용처 중 하나입니다:

1. 로그인 시, 서버는 쿠키를 설정하도록 하는 `Set-Cookie` HTTP 헤더(HTTP-header)를 전송합니다. 이 헤더엔 "세션 ID(session identifier)" 정보가 들어있습니다.
2. 브라우저는 응답을 받아 쿠키를 저장합니다.
3. 동일 도메인에 접속 시, 브라우저는 `Cookie` HTTP 헤더를 이용해 요청합니다.
4. 서버는 이를 이용해 요청을 식별합니다.

브라우저엔 쿠키를 위한 특별한 접근자인 `document.cookie`가 있습니다.
...
쿠키를 구성하는 문자열은 `name=value`의 짝으로 구성되어 있고, `; ` 로 구분됩니다. 하나의 짝은 하나의 독립된 쿠키입니다.

`; `을 기준으로 `document.cookie`를 분리하면 특정 쿠키를 찾을 수 있습니다. 분리엔 정규 표현식이나 배열 함수를 이용할 수 있습니다.

본 챕터의 뒷쪽에서 쿠키에 관련된 몇가지 함수를 살펴보고, 원하는 쿠키를 쉽게 찾을 수 있는 방법에 대해 알아보도록 하겠습니다.
...
위 코드를 실행하면 여러개의 쿠키가 출력되는걸 볼 수 있습니다. `document.cookie=` 연산은 코든 쿠키를 덮어쓰지 않고 `user`의 값만 갱신합니다. 
...
만약 `path=/mypath`로 설정되었다면, `/mypath` 와 `/mypath/*`에서 쿠키를 볼 수 있고, `/page` 나 `/mypathpage`에선 볼 수 없습니다.
...
서브 도메인(subdomain)인 `forum.site.com`에서도 마찬가지로 쿠키 정보를 얻을 수 없습니다. 이런 점이 쿠키를 다루는 걸 까다롭게 만듭니다.
...
`expires` 옵션의 대안으로, 쿠키 만료 기간을 설정할 수 있습니다. 현재부터 일정 초 이후로 값을 설정해서 말이죠.

옵션값은 기간을 초로 환산한 숫자가 될 수 있고, 0이나 음수가 될 수도 있습니다. 0이나 음수값을 설정하면 쿠키가 바로 삭제됩니다: 
...
하지만 `secure` 옵션이 붙어있으면, `https://site.com`에서 설정한 쿠키는 `http://site.com`에서 액세스할 수 없습니다. HTTP 연결이기 때문입니다. 만약 쿠키에 민감한 콘텐츠가 있고, 이 콘텐츠를 암호화되지 않은 HTTP 연결을 통해 전달하지 않고 싶다면, 이 옵션을 사용하면 됩니다.

```js
// HTTPS 연결에서만 쿠키를 전송할 수 있도록 설정
...
이제, (로그아웃하지 않고) 다른 창을 띄워서 웹 서핑을 하던 도중에 우연히 `evil.com`에 접속했다 가정해 봅시다. 이 사이트엔 해커의 계정에 지급 요청을 하는 `<form action="https://bank.com/pay">` 자바스크립트 코드가 포함되어 있고, 이 폼(form)은 자동으로 전송되도록 설정되어 있습니다. 
...
실제 은행엔 당연히 이 공격을 막을 수 있습니다. `bank.com`이 생성한 모든 폼에는 악의적인 페이지에선 만들 수 없고, 원격 페이지에서도 추출 할 수 없는 특수 필드인 "XSRF 보호 토큰(protection token)"이 있기 때문입니다(원격 사이트는 폼을 전송할 순 있지만, 데이터를 돌려 받을 순 없습니다).

### 쿠키의 samesite 옵션

쿠키의 `samesite` 옵션을 이용하면 (이론상으로) 크로스 사이트 요청 위조를 막을 수 있습니다. "XSRF 보호 토큰" 없이도 말이죠.
...
이 말은, 사용자가 메일에 있는 링크를 따라 접속했거나 `evil.com`과 같은 사이트에서 폼을 전송한 경우, 혹은 요청이 제3의 도메인에서 이루어졌을 때, 쿠키가 전송되지 않는다는 것을 의미합니다.
...
두 종류의 쿠키를 사용해 이 문제를 해결할 수 있습니다. 단순히 "Hello, John"과 같은 메시지를 출력해주는 "일반 인증(general recognition)"을 위한 쿠키를 하나 만들고, 데이터 교환 시 사용하는 `samesite=strict` 옵션이 있는 쿠키를 따로 둬서 말이죠.

이렇게 쿠키 두 개를 사용하면 외부 사이트를 통해 접근한 사용자는 정상적으로 환영 메시지를 볼 수 있게 됩니다. 이후의 프로세스인 지급은 무조건 은행의 사이트를 통해서만 수행되도록 만들면 됩니다.

- **`samesite=lax`**

`samesite=lax` 옵션은 느슨한(relaxed) 접근법으로, 사용자 경험을 해치지 않으면서 XSRF 공격을 막을 수 있는 또 다른 방법입니다.
...
2. 조작이 최상위 레벨 탐색에서 이루어 질 때(브라우저 주소 창에서 URL을 변경하는 경우).
    ...
    대다수는 이 조건을 충족합니다. 하지만 `<iframe>`안에서 탐색이 일어날 경우는 이 조건을 충족하지 못합니다. AJAX 요청 또한 탐색 행위가 아니므로 이 조건에 맞지 않습니다.

`samesite=lax` 옵션은 대다수의 일반적인 "open URL" 작업 시, 쿠키를 보낼 수 있도록 허용해 줍니다. 외부 사이트에서 AJAX 요청을 보내거나 폼을 보내는 등의 복잡한 작업 시는 쿠키를 전송하지 못하게 합니다.
...
하지만 `samesite`를 XSRF 토큰 같은 다른 보안 기법과 함께 사용한다면 보안을 더 강화할 수 있습니다.

## httpOnly

이 옵션은 자바스크립트와 전혀 관계가 없지만, 완결성을 위해 언급하고 넘어가도록 하겠습니다.

이 옵션은 서버 측에서 `Set-Cookie`를 통해 쿠키를 설정할 때, 지정할 수 있습니다.
...
유사한 기능을 하는 다양한 쿠키 라이브러리가 존재하긴 합니다. 실제로 동작하는 코드이므로, 데모 목적으로 봐주시면 될 것 같습니다.
...
위에 쓰인 정규식은 `; name=<value>` 패턴을 찾기 위해 사용되었습니다.

위에서 언급했듯이 쿠키값은 임의의 문자열입니다. 공백 이나 `;`같이 포맷팅 규칙을 위반하는 문자가 문자열 중간에 들어가 있다면 이런 문자들은 인코딩해 주어야 한다는 점을 잊지 마세요. 

인코딩한 문자열을 다시 디코딩 하려면 내장 함수, `decodeURIComponent`를 사용하면 됩니다. getCookie 함수는 이 내장함수를 사용하여 문자열을 다루고 있습니다.
...
4. 사용자가 `site.com`을 떠나 타 사이트인 `other.com`에 접속하고, 이동한 사이트에도 배너가 있으면, `ads.com`은 쿠키를 전송받습니다. 이 쿠키는 `ads.com`에서 설정한 것이기 때문이죠. 이를 이용해 `ads.com`은 사용자를 인식하고, 이 사용자가 어떤 사이트로 이동했는지를 추적합니다:
