...
<<<<<<< HEAD
`event.target`과 `this`가 같은 경우도 있습니다 -- `<form>` 요소를 정확히 클릭했을 때 같아집니다.
=======
...
3. `stopPropagation`로 버블링을 막아놓은 영역에선 이 코드가 동작하지 않기 때문에, 분석 시스템이 제대로 작동하지 않습니다. "죽은 영역(dead zone)"이 되어버립니다.
...
이벤트엔 버블링 이외에도 "캡처링" 이라는 흐름이 존재합니다. 실제 코드에서 자주 쓰이진 않지만, 종종 유용하게 쓰이는 경우가 있으므로 알아봅시다.
...
`on<event>`-프로퍼티, HTML 속성, `addEventListener(event, handler)`을 이용해 할당된 핸들러는 캡처링에 대해 전혀 알 수 없습니다. 이 핸들러들은 두 번째 혹은 세 번째 단계의 이벤트 흐름(타깃 단계와 버블링 단계)에서만 동작합니다.
