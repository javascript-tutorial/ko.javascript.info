...
아래 핸들러는 `<div>`에 할당되어 있지만, `<em>` 이나 `<code>`같은 중첩태그를 클릭해도 동작합니다:
...
**한 요소에 이벤트가 발생하면, 이 요소에 할당된 핸들러가 동작합니다. 그다음 그 부모 요소의 핸들러가 동작하고, 이 과정이 반복되면서 가장 최상단의 조상을 만날 때까지 각 요소에 할당된 핸들러가 작동합니다.**

3개의 요소가 중첩된 구조 `FORM > DIV > P`를 살펴봅시다. 각 요소엔 핸들러가 할당되어 있습니다:
...
4. 트리를 거슬러 올라가면서 `document` 객체를 만날 때까지 요소에 할당된 `onclick` 핸들러가 동작합니다.

![](event-order-bubbling.png)

이런 원리 때문에 `<p>` 요소를 클릭하면 3개의 alert 창을 볼 수 있는것 입니다. `p` -> `div` -> `form` 순서로 말이죠.
...
`focus` 이벤트는 버블링 되지 않고, 이 외에도 버블링 되지 않는 몇 가지 이벤트가 있습니다. 하지만 이런 이벤트는 많지 않습니다. 대부분 이벤트가 버블링 됩니다.  
```
...
캡처링 단계에서 이벤트를 잡아내려면 `addEventListener`의 세 번째 인수를 `true`로 설정해야 합니다.

`addEventListener`의 세 번째 인수는 옵션값인데, 2가지 값을 가질 수 있습니다:
...
타깃 요소에 캡처링과 버블링 핸들러를 할당했다면, 캡처링 단계의 마지막에 캡처 핸들러가 동작하고 버블링 단계의 마지막에 버블 핸들러가 동작합니다.
...
- 이벤트는 document 에서 시작해 DOM 트리를 따라 `event.target`까지 내려감. 이때, 트리 내 요소에 `addEventListener(...., true)`가 할당된 경우, 해당 핸들러가 동작함.
